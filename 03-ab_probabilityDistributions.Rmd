...
ggplot(data.frame(x = c(-5, 5)), aes(x)) +
  ...
  labs(x = "Value", y = "Cumulative Probability", title = "CDF of a continuous distributions") +
  theme_bw() +
...
where $f_X(x)$ is the density of the random variable $X$ evaluated at some point $x$ and the integral over $x$ has the same purpose as the sum over $x$ before. 

### Uniform Distribution

To illustrate the concept of the integral the uniform distribution provides a simple example. As in the continuous case it assigns equal weight to each equally sized between the area on which the variable is defined $[a, b]$. Why each interval and not each value? Since there are infinitely many values between $a$ and $b$ each individual value cannot be assigned a probability small enough for all of the probabilities to sum to $1$ (which is a basic requirement of a probability). Thus we can only assign a probability to an interval, e.g. $[0, 0.001]$, of which only *finitely* many exist between $a$ and $b$, e.g. $a = -2$ and $b = 1$. In this example there exist $3000$ intervals of values $[x, x + 0.001]$. Since we are dealing with intervals the density can be thought of as the area under the PDF for a given interval.
...
That is, the weight $\frac{1}{b-a}$ is assign to values in the interval of interest and all other values have weight $0$. As already mentioned *all* values between $a$ and $b$ have to be considered.  
