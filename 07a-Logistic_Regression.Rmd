...
$$
What this formula implies is that we are taking the probability predictions from our well known linear form and ensuring that they only result in values between 0 and 1, thus giving us valid probabilities. 
...
To investigate the model output, we can again use ```summary()```
...
STILL NEED TO INCLUDE INTERPRETATION OF MFX!
...
Measures for the absolute fit, comparable to the $R^2$ for linear regressions, are also available. In fact multiple "Pseudo $R^2$s" have been developed. There are multiple packages that return the $R^2$ given a logit model (see ```rcompanion``` or ```pscl```). The calculation by hand is also fairly simple. This is left as an exercise for the reader.
...
The prediction indicates that a 30 year old male who has been married for two years, has 16 years of education, highly rates his marriage and is not very religious has a probability of cheating of $\sim 24\%$ (Try predicting your own likelihood of cheating).   
