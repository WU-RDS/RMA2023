...
There are various similar packages with pre-specified themes, like the <a href="https://github.com/cttobin/ggthemr" target="_blank">`ggthemr`</a> package or the <a href="https://github.com/ricardo-bion/ggtech" target="_blank">`ggtech`</a> package. 
:::
...
Now, let's create a boxplot based on this variables and plot the log-transformed number of streams by genre. 
...
It is often meaningful to augment the boxplot with the data points using ```geom_jitter()```. This way, differences in the distribution of the varibale between the genres become even more apparent. 
...
::: {.infobox_orange .hint data-latex="{hint}"}
Alternatively, you may use one of the many user contributed packages to visualize your data. One example is the <a href="https://indrajeetpatil.github.io/ggstatsplot/index.html" target="_blank">ggstatplot</a> package, which also reports the results from statistical analysis, such as an ANOVA. See also the presentation <a href="https://indrajeetpatil.github.io/ggstatsplot_slides/slides/ggstatsplot_presentation.html#1" target="_blank">here</a>. You can find one example how to create a boxplot below. 
:::

```{r message=FALSE, warning=FALSE, echo=TRUE, eval=TRUE, fig.align="center", fig.cap = "Boxplot using ggstatsplot package"}
ggbetweenstats(
    data = music_data,
