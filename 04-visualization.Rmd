...
```{r message=FALSE, warning=FALSE,echo=TRUE, eval=TRUE, fig.align="center", fig.cap = c("Geo-location data (1)","Geo-location data (2)")}
index1 = 1
index2 = 1
success1 = FALSE
success2 = FALSE
while(index1<10|!success1){
  # Download the base map
  de_map_g_str <- try(get_map(location = "Germany", zoom = 6), silent = TRUE)
  if(!is(de_map_g_str, "try-error")){
    # Draw the heat map
...
while(index2<10|!success2){
    usa_center = as.numeric(geocode("Germany"))
    gm <- try(ggmap(get_googlemap(center = usa_center, scale = 2, zoom = 6), extent = "normal") +
      ...
      xlim(min(geo_data$lon), max(geo_data$lon)), silent = TRUE)
    if(!is(gm, "try-error")){
      gm
      success2 = TRUE
    }else{
    ...
    index2 = index2 + 1}
  if(!allSuccess){
    warning("Google Maps exited unsuccessfully")
  }
}
```
