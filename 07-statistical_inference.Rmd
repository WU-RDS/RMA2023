...
Observe that in this first draw the mean is quite close to the actual mean. It seems like the sample mean is a decent estimate of the population mean. However, we could just be lucky this time and the next sample could turn out to have a different mean. So in order to make sure that the first draw is not just pure luck and the sample mean is in fact a good estimate for the population mean, let us take **many** (e.g. $20,000$) different random samples and calculate their means. This will show us a range within which the sample mean of any sample we take is likely going to be. We are going to store the means of all the samples in a matrix and then plot a histogram of the means to observe the likely values. The sample size is $100$ for each sample that we take. 
...
As you can see, on average the sample mean ("mean of sample means") is extremely close to the population mean, despite only sampling $100$ people at a time. This distribution of sample means is also referred to as **sampling distribution** of the sample mean. However, there is some uncertainty, as visualized by the histogram. Let's look at the means from the first $5$ samples, as well as the minimum, the maximum, and the mean of the sample means:
...
As you can see, the means are slightly different for the different samples and range from `r round(min(means),2)` to `r round(max(means),2)`. The following plot shows the distribution of listening times for four different samples from the population, each containing responses from 100 randomly drawn students.  
...
The central limit theorem states that if **(1)** the population distribution has a mean (there are examples of distributions that don't have a mean , but we will ignore these here), and **(2)** we take a large enough sample, then the sampling distribution of the sample mean is approximatelly normally distributed. What exactly "large enough" means depends on the setting, but the interactive element at the end of this chapter illustrates how the sample size influences how accurately we can estimate the population parameters from the sample statistics.
...
The vertical black line represents the population mean ($\mu$), which is `r round(mean(hours),2)` hours. The following plot depicts the historam of listening times of one random sample of 100 students from the population: 
...
In summary, it is important to distinguish two types of variation: **(1)** For each individual sample that we may take in real life, the standard deviation ($s$) is used to describe the **natural variation** in the data and the data may follow a non-normal distribution. **(2)** If we would (hypothetically!) repeat the study many time, the sampling distribution of the sample mean follows a normal distribution for large samples sizes (even if data from each individual study are non-normal), and the standard error ($\sigma_{\bar x}$) is used to describe the **variation bewtween study results**. This is an important feature, since many parametric statistical tests assume a normally distributed sampling distribution. As we have seen, this does **not** mean that your data needs to follow a normal distribution.
...
We can see that 95% of the values are roughly within 1.96 standard deviations from the mean. Since we know the sample mean ($\bar x$) and we can estimate the standard deviation of the sampling distribution ($\sigma_{\bar x} \approx {s \over \sqrt{n}}$), we can now easily calculate the lower and upper boudaries of our confidence interval as:

$$
CI_{lower} = {\bar x} - 1.96 * \sigma_{\bar x} \\
CI_{upper} = {\bar x} + 1.96 * \sigma_{\bar x}
$$ 
