...
```lm(...)``` can take squared (or any power) terms as input by adding ```I(X^2)``` as explanatory variable. In the example below we see a clear quadratic relation ship with an inflection point at around 250. If we try to model this using the level of the covariates without the quadratic term we do not get a very good fit.

```{r}
X <- rnorm(1000, 0, 500)
Y <- 50 + 5*X - .01*(X^2) + rnorm(1000, 0, 500)

modLinear <- lm(Y ~ X)
ggplot(data.frame(outcome = Y, regressor = X, fit = fitted(modLinear))) +
  geom_point(aes(x = regressor, y = outcome, color = "Data")) +
  geom_line(aes(x = regressor, y = fit, color = "Fitted")) +
  theme_bw() +
  ggtitle("Linear Model") +
  theme(legend.title = element_blank())
```

By including the quadratic term we can fit the data very well. This is still a linear model since the outcome variable is still explained by a linear combination of regressors even though one of the regressors is now just a non-linear function of the same variable (i.e. the squared value).  

```{r}
mod <- lm(Y ~ X + I(X^2))
summary(mod)

ggplot(data.frame(outcome = Y, regressor = X, fit = fitted(mod))) +
  geom_point(aes(x = regressor, y = outcome, color = "Data")) +
  geom_line(aes(x = regressor, y = fit, color = "Fitted")) +
  theme_bw() +
