...
In order to make a decision on whether the difference can be deemed statistically significant, we now need to compare this calculated test statistic to a meaningful threshold. Recall that for the normal distribution, 95% of the total probability falls within `r round(qnorm(p2),2)` standard deviations of the mean, so that higher (absolute) values provide evidence against the null hypothesis. The critical value is therefore given by    
...
We use ```0.975``` and not ```0.95``` since we are running a two-sided test and need to account for the rejection region at the other end of the distribution. Generally, we speak of a statistically significant effect if the (absolute) calculated test statistic is larger than the (absolute) critical value. We can easily check if this is the case in our example:
...
**Decision:** Reject $H_0$, given that the p-value of `r round(p_value,2)` is smaller than 0.05. 
...
H_0: \mu_D = 0 \\
$$
Not that the hypothsis only refers to one population, since both observations come from the same units (i.e., users). To use consistent notation, we replace $\mu_D$ with $\delta$ and get:
