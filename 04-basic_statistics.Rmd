...
```{r message=FALSE, warning=FALSE, paged.print = FALSE}
test_data %>% mutate(duration_std = as.vector(scale(duration))) %>% filter(duration_std > 3)
```
...
```{r message=FALSE, warning=FALSE, paged.print = FALSE}
library(dplyr)
estimation_sample <- test_data %>% mutate(duration_std = as.vector(scale(duration))) %>% filter(duration_std < 3)
