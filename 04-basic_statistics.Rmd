...
```{r message=FALSE, warning=FALSE, paged.print = FALSE}
library(dplyr)
music_data %>% mutate(adv_spending_std2 = as.vector(scale(adv_spending2))) %>% filter(abs(adv_spending_std2) > 3) %>% select(id, trackName, adv_spending2, adv_spending_std2)
```
...
```{r message=FALSE, warning=FALSE, paged.print = FALSE}
library(dplyr)
estimation_sample <- music_data %>% mutate(adv_spending_std2 = as.vector(scale(adv_spending2))) %>% filter(abs(adv_spending_std2) < 3)
psych::describe(estimation_sample[,c("trackPopularity", "adv_spending2")])
```
